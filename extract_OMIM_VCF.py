...
for pathway_name in ["insulin signaling", "tryptophan metabolism"] :
    ...
    OMIM_variants_filename = "%s/OMIM_gene_variants_in_VCF.txt" % pathway_folder
    if os.path.exists(OMIM_variants_filename): os.remove(OMIM_variants_filename)
    ...
    for alleleIDs_file in sorted(glob("%s/*_alleleIDs.txt" % OMIM_folder)):
        ...
        with open(alleleIDs_file, "r") as in_allele_ids, open(OMIM_variants_filename, "a") as out_clinvar :
